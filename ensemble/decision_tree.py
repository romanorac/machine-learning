...
Algorithm builds a binary decision tree. It expands nodes in priority order, where priority is set by measure
...
def rand_indices(x, rand_attr):
    ...
    :param rand_attr - parameter defines number of randomly selected features
    """
    loop = True
...
def fit(x, y, t, randomized, max_tree_nodes, min_samples_leaf, min_samples_split, class_majority, measure,
        separate_max):
    """
    ...
    :param x numpy array - dataset with features
    :param y numpy array - dataset with labels
    :param t list - features types
    :param randomized: boolean - if True, algorithm estimates sqrt(num_of_features) randomly selected features each
            iteration. If False, it estimates all features in each iteration.
    :param max_tree_nodes - number of tree nodes to expand,
    :param min_samples_leaf: int: - minimal number of samples in leafs,
    :param min_samples_split: int: - minimal number of samples to split a node,
    :param class_majority: float - purity of the classes in leafs,
    :param measure: measure function,
    :param separate_max: bool: separate most common nominal attribute from others
    ...
    while recalculate:
        ...
        est = [measure(x[:, i], y, t[i], separate_max) for i in est_indices]
        try:
    ...
    while not queue.empty() and len(tree) * 2 < max_tree_nodes:
        ...
        for j in range(2):  # for left and right branch of the tree
            ...
            if current_majority < class_majority and len(sub_y) > min_samples_split and est_indices != -1:
                ...
                est = [measure(sub_x[:, i], sub_y, t[new_mapping[i]], separate_max) for i in est_indices]
                try:
...
def predict(tree, x, y=[], dist=False):
    ...
    :param tree: dictionary - tree model
    :param x numpy array - one sample from the dataset
    :param y string, integer or float - sample label
    :param dist bool, return distribution or prediction
